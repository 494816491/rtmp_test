...
static int add_audio_stream(struct rtmp_h264_st *rtmp_h264_data)
{
    ...
    if(rtmp_h264_data->audio_stream == NULL){
        printf( "avformat_new_stream error,audio_stream = %p\n", rtmp_h264_data->audio_stream);
        return -1;
    }
...
int set_rtmp_chn_param(int chn, struct rtmp_chn_param_st *param)
{
    if(chn >= RTMP_H264_CHN_NUM){
        printf("chn can not larger than RTMP_H264_CHN_NUM");
        exit(-1);
    }
...
int open_rtmp_stream(int chn)
{
    int ret;
    struct rtmp_h264_st *rtmp_h264_data = &rtmp_h264_info[chn];
    printf("open_rtmp_stream %s", rtmp_h264_data->rtmp_ip);
    pthread_mutex_lock(&rtmp_h264_data->mutex);
    ...
    if (!(rtmp_h264_data->out_context->oformat->flags & AVFMT_NOFILE))
    {
        ...
        printf("avio_open2, rtmp_h264_data->rtmp_ip = %s", rtmp_h264_data->rtmp_ip);
    ...
    if (ret < 0) {
        printf( "Fail to write the header of output ");
        goto err_label;
    }
    pthread_mutex_unlock(&rtmp_h264_data->mutex);
    return 0;
...
int watch_rtmp_video_audio_is_alive(int chn)
{
    ...
    if(rtmp_h264_data->out_context){
        printf("watch_rtmp_video_audio_is_alive , rtmp stream is ok");
    }else{
        printf("watch_rtmp_video_audio_is_alive failed, restart rtmp stream");
...
{
    struct rtmp_h264_st *rtmp_h264_data = &rtmp_h264_info[chn_num];
    int ret;

    //printf("before send_rtmp_video_stream");
    ...
    if(ret != 0){
        ...
        close_rtmp_stream(chn_num);
    }

unlock_label:
    pthread_mutex_unlock(&rtmp_h264_data->mutex);

    //printf("after send_rtmp_video_stream");
...
int send_rtmp_video_stream(Mal_StreamBlock *block)
{
    ...
    //printf("pkt.data = %p, size = %d, stream_index = %d\n", pkt.data, pkt.size, pkt.stream_index);
    //printf("rtmp_h264_data->out_context = %p", rtmp_h264_data->out_context);
...
{
    //printf("get_raw_stream_a video_chn = %d\n", block->chn_num);
    ...
    if(pkt.size == 0){
        printf("resample_main no buffer size");
        return 0;
    }
#endif
    ...
    ret = av_interleaved_write_frame(rtmp_h264_data->out_context, &pkt);
    if(ret != 0){
        printf("av_interleaved_write_frame error audio,chn_num = %d\n",rtmp_h264_data->chn_num);
...
int close_rtmp_stream(int chn)
{
    ...
    pthread_mutex_lock(&rtmp_h264_data->mutex);
    printf("before close_rtmp_stream");
    if(rtmp_h264_data->out_context){
...
#endif
        avformat_flush(rtmp_h264_data->out_context);
...
static int resample_main(char *g711a_data, int g711a_data_size, char **ret_buf, int *ret_buf_size)
{
    ...
    if(g711a_data_size != info->src_nb_samples){
        printf("g711a_data_size != info->src_nb_samples , g711a_data_size = %d", g711a_data_size);
        exit(-1);
    }
